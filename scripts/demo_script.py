...
def main():
    ...
    node_id = IRI("https://www.sfb1574.kit.edu/ontologies/TransferUnit")
    ...
    print(f"  - Node model class: {node.type()}")
    ...
    loaded_instance = node.load(ogm.loader)
    ...
    print(f"  - Both nodes share the same model class: {node.type() is node2.type()}")
    node3 = ogm.create_node(id=node_id, model_cls=node.type())
    ...
    node5 = ogm.create_node(id=node_id, model_cls=node.type(), instance=loaded_instance)
