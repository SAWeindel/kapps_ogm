...
class NodePropertyChainExtractor:
    """Extracts property chains from nested Node data structures."""

    def __init__(self, node: Node):
        """
        Initialize the extractor with a Node instance.

        Args:
            node: The Node instance to extract property chains from.
        """
        self.node = node

    def extract(self) -> list[list[IRI | str]]:
        """
        ...
        def normalize_key(key: Union[str, IRI]) -> IRI | str:
            ...
            if self.node.class_spec:
                model = self.node.class_spec.to_pydantic_model()
