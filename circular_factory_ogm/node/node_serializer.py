...
class NodeSerializer:
    """Handles serialization of Node instances to various RDF formats."""

    @staticmethod
    def to_triples(node: Node) -> set[Triple]:
        ...
        iri_field_map: dict[str, IRI] = getattr(
            node.instance.__class__, "_iri_fields", {}
        )

        # We need either a ClassSpec or _iri_fields to know how to serialize
        if node.class_spec is None and not iri_field_map:
            raise RuntimeError(
        ...
        if node.class_spec and node.class_spec.iri:
            triples.add((subject, "rdf:type", node.class_spec.iri))
        ...
        for field_name, prop_iri in iri_field_map.items():
            value = getattr(node.instance, field_name, None)
            if value is None:
                continue
            ...
            for v in values:
                triples |= NodeSerializer._value_to_triples(
                    node=node,
                    subject=subject,
                    predicate=prop_iri,
                    value=v,
    ...
    @staticmethod
    def to_json_ld(
        node: Node, context: Optional[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        triples = NodeSerializer.to_triples(node)
    ...
    def _value_to_triples(
        ...
        if isinstance(value, BaseModel):
            ...
            if nested_id:
                ...
                obj = BNode(node.ogm.db.new_blank_id())
            ...
            triples |= NodeSerializer.to_triples(nested_node)
